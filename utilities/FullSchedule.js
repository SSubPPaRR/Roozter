$(document).ready(function () {
    ...
    $("#date").change(function (e) {
        ...
        var date = $("#date").val();
        console.log(date);
        $.post("utilities\\loadFullSchedule.php", {
            date: date
        },
            function (data) {
    ...
    $("#add-event-btn").click(function (e) {
        ...
        $("#add-event-form").show();
        /*
        var facid = $("#add-event-form .Faculty").val();
    ...
    $("body").on("change", ".del-checkbx", function (e) {
        e.preventDefault();
        var total = $('.del-checkbx:checked').length;
        //console.log(total);
        if (total > 0) {
            $("#delete-sel-event-btn").removeAttr("disabled");
        } else $("#delete-sel-event-btn").attr("disabled", "disabled");
    });

    //delete selected / delete all

    //delete all         
    $("#delete-all-event-btn").click(function (e) {
        e.preventDefault();
        $("#delete-confirmbox2").show();
        $.get("full-schedule.php",
            function (data) {
                $("#confirmbox-btns2").replaceWith($(data).find("#confirmbox-btns2"));
            }
        );
        $("#delete-type").val($(this).attr("data-delete-type"));
        $("#delete-confirm-message").html("Are you sure you want to delete all events from schedule?<p>(Note: this will delete everything!)</p>");
    });

    //delete selected
    $("#delete-sel-event-btn").click(function (e) {
        e.preventDefault();
        var total = $('.del-checkbx:checked').length;
        ...
        $("#delete-type").val($(this).attr("data-delete-type"));
        $("#delete-confirm-message").html("Are you sure you want to the " + total + " selected events from schedule?");
    });

    //cancel
    $("body").on("click", "#delete-confirmbox2 .cancel-btn", function () {
        document.getElementById('confirm-form2').reset();
        $("#delete-confirmbox2").hide();
    });

    //delete
    $("body").on("click", "#delete-confirmbox2 .delete-btn", function () {

        var option = $("#delete-type").val();
        $("#confirmbox-btns2").replaceWith('<div id="confirmbox-btns2" class="w3-center" style="width: 100%;"><img style="width: 50px;" src="images\\Infinity-1s-200px.svg"></div>');
        //1 =all / 2= selected
        if (option == 1) {
            ...
            $.post("utilities\\deleteEvent.php", {
                ...
                function (data) {
                    $("#confirmbox-btns2").replaceWith(data);
                    refreshSchedule();
                }
            );
            ...
            $(".del-checkbx:checked").each(function (index, element) {
                // element == this
                deleteIds.push($(this).attr("data-id"));
            });
            var ids = deleteIds.toString();
            console.log(ids);
            ...
            $.post("utilities\\deleteEvent.php", {
                ...
                function (data) {
                    $("#confirmbox-btns2").replaceWith(data);
                    $("#delete-sel-event-btn").attr("disabled", "disabled");
                    refreshSchedule();
                }
            );
        }
    ...
    $("#sel-all").click(function (e) {
        ...
        console.log(total);
        if (total == 0) {
            $(".del-checkbx").each(function (index, element) {
                // element == this
                if ($(this).closest("tr").css("display") != "none") {
                    $(this).prop('checked', true);
                }
            });
            $("#delete-sel-event-btn").prop("disabled", false);

        } else {
            $(".del-checkbx").prop('checked', false);
            $("#delete-sel-event-btn").prop("disabled", true);
        }
